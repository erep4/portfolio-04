...
class Select extends BaseComponent {
  ...
  fixDropdownPosition() {
    ...
    const halfVeiwportX = viewportWidth / 2;
    const { width, x } = this.buttonElement.getBoundingClientRect();
    const buttonCenterX = x + width / 2;
    const isButtonOnTheLeftViewportSide = buttonCenterX < halfVeiwportX;
  ...
  onKeyDown = (event) => {
    ...
    const action = {
      ...
      Space: this.onArrowSpaceKeyDown,
      Enter: this.onArrowEnterKeyDown,
    }[code];
